...
const Template2 = () => (
    <div className='p-10 bg-white shadow-2xl border-2 rounded-lg w-[24cm] max-h-[29.7cm]'>
        <div className='flex flex-col mb-10'>
            <p className='text-blue-900 text-6xl font-bold'>{dev.basic.name}</p>
            <p className='text-gray-500 text-3xl font-bold mt-3'>{dev.basic.title}</p>
        </div>
                ...
                <div>
                    {dev.skills.map(({ name }, index) => (
                        <div className='inline-block' key={index}>
                            ...
                            <span className='pr-4 text-gray-400 text-lg'>{dev.skills.length !== index + 1 && '|'}</span>
                        </div>
                    ))}
                </div>
            </div>
